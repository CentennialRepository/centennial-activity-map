// server.js — Centennial Activity Map (production-optimized)
// - Redis session store in production (REDIS_URL)
// - Safe cookie handling behind proxies (Render)
// - /api/config route added
// - Static assets + protected SPA fallback
// - Auto-open only on local dev (no open() on Render)

// NOTE: This file uses top-level await and ES module imports.
...
// cookie policy: secure only when behind HTTPS / production
const cookieOpts = {
...
async function syncIfStale({ forceFull = false, forceSync = false } = {}) {
  ...
  try {
    ...
    return { synced: false, mode: patMode ? "airtable" : "csv", error: e.message || String(e) };
  }
}
...
// -------------------- Start server (auto-open only in local dev) --------------------
